...
describe('UFixed18', () => {
  ...
  describe('#from(Fixed18)', async () => {
    ...
    it('reverts if negative', async () => {
      await expect(uFixed18['from(int256)'](utils.parseEther('-10')))
        .to.be.revertedWithCustomError(uFixed18, 'UFixed18UnderflowError')
        .withArgs(utils.parseEther('-10'))
    })
  })
  ...
  describe('#from(UFixed18,int256)', async () => {
    it('creates new from significand and exponent', async () => {
      expect(await uFixed18.fromSignificandAndExponent(utils.parseEther('10'), 6)).to.equal(utils.parseUnits('10', 24))
    })

    it('creates new from significand and exponent with negative exponent', async () => {
      expect(await uFixed18.fromSignificandAndExponent(utils.parseEther('10'), -2)).to.equal(utils.parseEther('0.1'))
    })

    it('creates new from significand and exponent with zero significand', async () => {
      expect(await uFixed18.fromSignificandAndExponent(0, 6)).to.equal(0)
      expect(await uFixed18.fromSignificandAndExponent(0, 0)).to.equal(0)
    })

    it('creates new from significand and exponent with zero exponent', async () => {
      expect(await uFixed18.fromSignificandAndExponent(utils.parseEther('10'), 0)).to.equal(utils.parseEther('10'))
    })

    it('creates new from significand and exponent with large exponent', async () => {
      expect(await uFixed18.fromSignificandAndExponent(utils.parseEther('10'), 18)).to.equal(utils.parseUnits('10', 36))
    })

    it('creates new from significand and exponent with large significand', async () => {
      const LARGE_SIGNIFICAND = utils.parseUnits('1', 40)
      expect(await uFixed18.fromSignificandAndExponent(LARGE_SIGNIFICAND, 1)).to.equal(LARGE_SIGNIFICAND.mul(10))
    })

    it('reverts if too large', async () => {
      const TOO_LARGE = ethers.constants.MaxInt256
      await expect(uFixed18.fromSignificandAndExponent(1, TOO_LARGE)).to.be.reverted
    })

    it('reverts if too small', async () => {
      const TOO_SMALL = ethers.constants.MinInt256.add(1)
      await expect(uFixed18.fromSignificandAndExponent(1, TOO_SMALL)).to.be.reverted
    })
  })
  ...
  describe('#div', async () => {
    ...
    it('reverts', async () => {
      await expect(uFixed18.div(0, 0)).to.revertedWithPanic(PANIC_CODES.DIVISION_BY_ZERO)
    })

    it('reverts', async () => {
      await expect(uFixed18.div(utils.parseEther('20'), 0)).to.revertedWithPanic(PANIC_CODES.DIVISION_BY_ZERO)
    })
  })
  ...
  describe('#divOut', async () => {
    ...
    it('reverts', async () => {
      await expect(uFixed18.divOut(0, 0)).to.revertedWithCustomError(uFixed18, 'DivisionByZero')
    })

    it('reverts', async () => {
      await expect(uFixed18.divOut(utils.parseEther('20'), 0)).to.revertedWithCustomError(uFixed18, 'DivisionByZero')
    })
  })
  ...
  describe('#muldivOut', async () => {
    ...
    it('reverts', async () => {
      ...
      ).to.revertedWithCustomError(uFixed18, 'DivisionByZero')
    })
  ...
  describe('#inside', async () => {
    context('returns true', async () => {
      it('if inside', async () => {
        expect(
          await uFixed18.inside(utils.parseEther('123.456'), utils.parseEther('123'), utils.parseEther('124')),
        ).to.equal(true)
      })

      it('if equal to min', async () => {
        expect(
          await uFixed18.inside(utils.parseEther('123'), utils.parseEther('123'), utils.parseEther('125')),
        ).to.equal(true)
      })

      it('if equal to max', async () => {
        expect(
          await uFixed18.inside(utils.parseEther('125'), utils.parseEther('124'), utils.parseEther('125')),
        ).to.equal(true)
      })
    })
    ...
    context('returns false', async () => {
      it('if outside', async () => {
        expect(
          await uFixed18.inside(utils.parseEther('123.456'), utils.parseEther('124'), utils.parseEther('125')),
        ).to.equal(false)
      })
    })
  })
  ...
  describe('#outside', async () => {
    context('returns true', async () => {
      it('if outside', async () => {
        expect(
          await uFixed18.outside(utils.parseEther('123.456'), utils.parseEther('124'), utils.parseEther('125')),
        ).to.equal(true)
      })
    })
    ...
    context('returns false', async () => {
      it('if inside', async () => {
        expect(
          await uFixed18.outside(utils.parseEther('123.456'), utils.parseEther('122'), utils.parseEther('124')),
        ).to.equal(false)
      })

      it('if equal to min', async () => {
        expect(
          await uFixed18.outside(utils.parseEther('123'), utils.parseEther('123'), utils.parseEther('125')),
        ).to.equal(false)
      })

      it('if equal to max', async () => {
        expect(
          await uFixed18.outside(utils.parseEther('125'), utils.parseEther('124'), utils.parseEther('125')),
        ).to.equal(false)
      })
    })
  })
