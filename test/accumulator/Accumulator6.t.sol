...
contract Accumulator6Test is Test {
    ...
    function _value() private view returns (int256) {
        return Fixed6.unwrap(acc.accumulator()._value);
    }
    ...
    function test_revertsIfIncrementNonzeroAmountZeroTotal() public {
        ...
        acc.increment(Fixed6.wrap(1), UFixed6Lib.from(0));
    }
    ...
    function test_revertsIfDecrementNonzeroAmountZeroTotal() public {
        ...
        acc.decrement(Fixed6.wrap(1), UFixed6Lib.from(0));
    }
    ...
    function test_returnsAccumulatedNoRounding() public {
        ...
        Fixed6 accumulated = acc.accumulated(from, UFixed6Lib.from(5));
        assertEq(Fixed6.unwrap(accumulated), 2e6);
    }

    function test_returnsPositiveAccumulatedRoundingDown() public {
        Accumulator6 memory from = acc.accumulator();
        acc.increment(Fixed6.wrap(1), UFixed6Lib.from(1));
        Fixed6 accumulated = acc.accumulated(from, UFixed6.wrap(1));
        assertEq(Fixed6.unwrap(accumulated), 0);
    }

    function test_returnsNegativeAccumulatedRoundingDown() public {
        Accumulator6 memory from = acc.accumulator();
        acc.decrement(Fixed6.wrap(1), UFixed6Lib.from(1));
        Fixed6 accumulated = acc.accumulated(from, UFixed6.wrap(1_000001));
        assertEq(Fixed6.unwrap(accumulated), -2);
    }
